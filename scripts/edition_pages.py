...
def page_svg(number, window, table, boards, chapter_title):
    esc = html.escape
...
def preview(edition, output):
    ...
    for number, window in enumerate(windows, 1):
        svg = page_svg(number, window, table, boards, titles[window["chapter"]])
        ET.fromstring(svg)
    ...
    document = ('<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
                ...
                f'<p>{len(windows)} pages · {len(table)} beats · {drawn} storyboarded.</p><section>' +
                "".join(c if c.startswith("<figure") else "</section>" + c + "<section>" for c in cards) + '</section></main></html>\n')
    (output / "index.html").write_text(document)
    print(f"{len(windows)} pages; {drawn}/{len(table)} beats storyboarded; wrote {output.relative_to(ROOT) if output.is_relative_to(ROOT) else output}/index.html")
