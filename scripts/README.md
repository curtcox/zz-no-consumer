...
| `text_budget.py report` / `check` | used versus budgeted lettering words at panel, page, chapter and overall levels; distributions, sparse/full rankings, lettering-type totals, element slots, page/chapter filters, what-if allowances, and JSON/TSV exports. Panel word shares are explicitly estimated from the page guideline. `check` runs offline accounting fixtures and reconciles the current census with `panels.py`; it does not judge whether a page needs more text. See [the manual](../README.md#lettering-budget-reports). |
