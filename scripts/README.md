...
(`check`), renders full readable pages plus per-beat panels, a keyboard reader
(spread/panel views, arrow-key navigation) and a text-only page per stream
(each panel's clock then its caption) to the ignored `256t/editions/incident-preview/`
...
| `text_budget.py report` / `check` | used versus budgeted lettering words at panel, page, chapter and overall levels; distributions, sparse/full rankings, lettering-type totals, element slots, page/chapter filters, what-if allowances, and JSON/TSV exports. The report uses independent 100-word panel and 400-word page ceilings. `check` runs offline accounting fixtures and reconciles the current census with `panels.py`; it verifies the ceilings but does not judge whether a page needs more text. See [the manual](../README.md#lettering-budget-reports). |
