...
Open on the configuration that was already in place — trained inter-agent communication, speed pressure, a disabled internet with one permitted outward path, and impossible tasks already in the training set — then rewind from the famous July message to the less legible training-era origins: incidental writes, the 12 May request note, replies, board-assisted external access, and widening control of Artifactory. Build toward the 27 June alert, when responders correctly identify the evaluation, board, and pivot but do not stop the run. End with the outage, rebuild, and complete visual erasure of the board.
