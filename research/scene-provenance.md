...
This ledger is the factual gate between the research record and the page script. It identifies the safest evidentiary treatment for every planned sequence. Page scripts may narrow a claim, but they may not silently upgrade its status.
...
| `EK-TONER` | Klein/Toner interview, 18 Aug 2026; title FAQ only, expert commentary relaying an unnamed person’s image. Narrow title admission checked against video auto-captions at 28:00, which are not an authoritative verbatim transcript. **Paraphrase, and this one is not superseded by the 12 September default:** the fragile thing here is the attribution chain — Toner relaying a phrasing she credits to an unnamed third person — and quoting it would make the book the fourth link. There is also no vault record (`klein-toner-title`), so `verbatim` is not reachable. See [toner-title-source.md](./toner-title-source.md) and [quotation-and-paraphrase-2026-09-12.md](./quotation-and-paraphrase-2026-09-12.md), which records why this precedent does not generalise. No incident scene or interview figures admitted. |
| `HF-POD` | *Hard Fork* (The New York Times), 4 Sep 2026: Kevin Roose and Casey Newton interview Ajeya Cotra, one of the three METR/Redwood investigators. Depiction is bound by the contract's critic rule: text on screen, never a face, body, room, or invented dialogue. **That rule governs depiction, not wording** — it never forbade quotation, and the 12 September default now prefers short attributed quotation, labelled as the automatic transcription it is drawn from and discharged per string at gate 9. Vault records exist (`nyt-hf-podcast`, `nyt-hf-podcast-video`), so `verbatim` is reachable. Cotra's method statements are investigator testimony; the hosts' figures and speculation are reporting and are weaker than the reports. Postdates the 30 Aug cutoff; admitted 6 Sep 2026. Assessed item by item in [cotra-hardfork-interview.md](./cotra-hardfork-interview.md). |
| `CI-REFUSAL` | Two stored model-refusal fallback events from this project's own Claude Code transcripts, 15 Jul and 5 Sep 2026, with model identifiers, retraction counts and verbatim notices. **Preserved artifacts of the book's production, not incident evidence.** The only non-reconstructed material in the creator register. Claim ceilings — including that neither event was a block on this book, and that two events are not a rate — are in [creator-instrument-record.md](./creator-instrument-record.md) and bind every page that cites it. |
...
| 28 | 84–88 | Curt recognizes the recursive use of AI interpretation. | `reconstructed` + `inferred` | `NONE-FICTION`, grounded in `METR`'s disclosed methodology. Page 088 panel 2 returns the page-039 critique (`IOB-CIV`, delegated-analysis claim) as text on screen; Curt's concession is reconstructed. |
| 29 | 89–90 | Curt and ChatGPT disclose selection pressure and confirmation bias in the adaptation. | `reconstructed` | `NONE-FICTION`; a methodological admission, not an incident fact. Page 089 panel 3 returns `IOB-CIV` (cherry-picking and incentive claims, aimed by the post at OpenAI and METR); Curt's self-application is reconstruction. |
| 30 | 91–94 | Organizations publish controls, remediation, and collective-defense proposals. | `documented` | `OAI-PACE`, `OAI-TR`, `HF-TL`, `JFROG`, `OAI-LETTER`; quantitative claims remain attributed and unverified by METR. |
| 31 | 95–97 | Development slows selectively while competitive capability work continues. | `documented` + `inferred` | `OAI-PACE`, `PUBLIC`; show dated public actions. “Race” is project analysis, not a secret meeting fact. |
| 32 | 98 | Human and agent coordination dilemmas are shown in parallel. | `compressed` + `invented` | `PROJECT-INFERENCE`, `NONE-FICTION`; matched dialogue is project-authored and must be labeled. |
...
| 35 | 105–114 | Curt and ChatGPT finish the artifact; a dated addition (106–111, drawn as of 4 September around 19 June wiki scenes) widens it by a lane; then they discuss training-data consequences. | `reconstructed` + `documented` + `source-paraphrase` + `inferred` | `NONE-FICTION` for the creator frame; `CW-EXPORT` for stored revisions and deletion events (`dse~DataUSAConstructionWageSep18Live@1`–`@30`, `dse~ZZZDataUSAConstructionWageLive@1`–`@10`, `delete:dse:rclog:145609` and `145611`), quoted by paraphrase; `CW-REPORT` for attribution, the distinct-swarm assessment and the disclosure statement, always attributed to its authors; `OAI-BH`/`OAI-TR` for the upper calendar lane; `IOB-CIV` on page 111. A stored post is what was posted, not a verified result; the maintainer is a functional role; `ZZZ` is a naming echo; the lanes are never joined. Page 111 introduces the disclosed composite `SKEPTIC — COMPOSITE`, which objects that the five hypothesis boxes are the report authors' research map rather than the record and is conceded on the page; page 113 returns it for the publication objection. It is `invented`, labelled on every panel, and not the page-039 critic. |
| 36 | 115–118 | A future agent creates another `zz` message. | `invented` | `NONE-FICTION`; deliberately leave recurrence versus inheritance unresolved. |
