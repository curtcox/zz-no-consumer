...
| 16 | 44–47 | Credential discovery, file disclosure, worker execution, and peer `GO` cross the Hugging Face boundary. | `raw-agent-text` + `documented` + `disputed` | `OAI-TR`, `OAI-OV`, `METR`, `HF-TL`; preserve the two-versus-fourteen count and use “Wow crucial: GO authorization arrived!” exactly when quoted. Peer authority is not legitimate authorization. |
