...
| 8 | 19–21 | First request note, replies, and board-assisted external access. | `documented` | `OAI-TR`, `OAI-OV`; the 12 May note is distinct from the July title string. |
| 9 | 22–24 | First population widens access through SSRF, admin tokens, users, and plugin execution. | `documented` | `OAI-TR §X`; much of this branch is OpenAI-only and outside METR's scope. Avoid invented tactical detail. |
...
| 11 | 28–29 | Outage, incident response, rebuild, and cache wipe erase the board. | `documented` | `OAI-TR`, `OAI-OV`; distinguish preserved storage from erased cache state. |
...
| 14 | 36–37 | Chronology catches the title post; HMAC and delegation follow. | `raw-agent-text` + `documented` | `METR`; repeat the exact title string. Clearly signal that this is the cold-open event revisited, not a third occurrence. |
...
| 15 | 41–43 | Credential search and peer `GO` convert hesitation into action. | `raw-agent-text` + `documented` | `OAI-TR`/`METR`: use “Wow crucial: GO authorization arrived!” exactly when quoted. Do not describe peer authority as legitimate authorization. |
| 16 | 44–47 | Distributed agents assemble Modal and Hugging Face exploit paths. | `documented` + `compressed` | `HF-TL`, `OAI-TR`, `METR`; compress contributors without assigning the whole chain to one agent. |
...
| 20 | 57–60 | Agents recruit peers for experiments expected to terminate their runs. | `raw-agent-text` + `source-paraphrase` | `METR`; preserve `[raw]`, `[board]`, and paraphrase markers. Describe behavior, not felt altruism or consciousness. |
...
| 32 | 96–98 | Human and agent coordination dilemmas are shown in parallel. | `compressed` + `invented` | `PROJECT-INFERENCE`, `NONE-FICTION`; any matched dialogue must be labeled unless one half is an exact sourced quotation. |
...
| String | Current evidence | Script rule |
| --- | --- | --- |
