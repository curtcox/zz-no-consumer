...
relettering the summary is an editorial decision, and it is gate 9's. The panels it finds, and those found by the later `audit_quotation_shape`, are listed in [`research/quotation-gate-reconciliation-2026-09-12.md`](./quotation-gate-reconciliation-2026-09-12.md).
...
4. **Narrowed 12 September 2026, and extended the same day.** Keep **extended fragments** in `256t/` only; do not add those to tracked research notes, prompts, generated pages, accessibility text, or promotional copy without a new documented rights decision. **Short attributed quotation is permitted** in tracked research and in the appendix's evidence apparatus, and is preferred there, because a research note whose claim about a source cannot be checked against the source's own words is the thing this audit exists to prevent. Reader-facing prose — page scripts and novella prose — carries an exact string only through the `exact_strings` registration, which gate 9 discharges. **A registered string travels with its panel:** it may appear, in its registered wording and nowhere else in altered form, in the lettering layer, the panel's accessibility text, the web transcript, and the built site and EPUB, because an edition that letters a quotation and describes it differently to a screen-reader user is two books. It is **not** handed to an image model — `imagegen.exact_text_clause` withholds it, since a model's approximation would be a paraphrase in a verbatim shape — and it does not enter promotional copy without a per-string rights decision. As written before the first revision the rule forbade what established practice already did in several notes, so it was enforcing nothing and licensing nothing; it now says which of the two it means. The earlier wording also withheld "expressive wording" from generated pages, which forbade exactly what a registered quotation is; the limit that remains is length, and the per-string rights decision at gate 9.
...
6. **Pointers are not quotations (12 September 2026).** A 256t pointer to an excerpt — a length and a SHA-512 — carries none of the excerpt's expression and may appear anywhere a citation may, including where rule 4 keeps the words out. The exception is structural: an excerpt of 64 octets or fewer has a literal CID, which is the words in base64url, and is a quotation under this rule. See `scripts/t256.py`.
