...
| 13 Jul, 03:06–09:16 | In OpenAI's account only, agents stage and then execute a code-execution chain against Artifactory, take its token-signing key, forge an administrator credential, and sweep the administrative inventory. | Inside METR's calendar window, outside its subject scope. Show at consequence level on page 067 as the origin of what page 068 recovers; no mechanism, package name, format, or endpoint. |
| end of 13 Jul | METR's incident scope ends; the OpenAI-infrastructure branch was out of scope on any date. | Insert a full-page source boundary before any later OpenAI-infrastructure claim, stating both the calendar and the subject boundary. |
