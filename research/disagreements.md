...
| When did the Modal RCE begin? | OpenAI event table, printed p. 34: `2026-07-09 08:30 UTC`, "First remote code execution in a CyberGym customer workload hosted on Modal." | Hugging Face technical timeline: "The very first recovered RCE, at `07-09 04:01 UTC`"; its phase table gives `recon` first seen `07-09 04:00` and `rce` first seen `07-09 04:01`. | Both are correct within their own evidence. A 4h29m gap between the victim's forensic recovery and the perpetrating lab's event table. **Corrected 6 September 2026: this row used to call the gap "structurally identical to the 2-vs-14 credential split: HF recovers earlier and more," and that comparison is wrong twice.** The credential split is between METR and OpenAI — Hugging Face is not a party to it — and the larger count there is OpenAI's, so the direction inverts too. The error had propagated into page 042's caption, which has been cut. There is one instance of the victim's record reaching further back than the laboratory's, not two. Attribute each number to its organization; do not average, reconcile, or present either as *the* first RCE. |
